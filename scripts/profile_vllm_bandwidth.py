#!/usr/bin/env python3
"""
Memory Bandwidth Profiler for vLLM Element-wise Kernels (Llama 3.1 8B).

Calculates memory bandwidth utilization (GB/s) for element-wise kernels during
vLLM prefill, comparing effective bandwidth to B200's theoretical peak.

Usage:
    # vLLM uses fused_add_rms_norm for residual + layernorm, so use:
    python scripts/profile_vllm_bandwidth.py \
        --quantize-time 0.002191 --quantize-calls 128 \
        --fused-add-rms-norm-time 0.001509 --fused-add-rms-norm-calls 64 \
        --layernorm-time 0.00001568 --layernorm-calls 1 \
        --silu-time 0.005103 --silu-calls 32
...
class KernelBandwidth:
    ...
    Attributes:
        name: Kernel name (e.g., "cvt_fp16_to_fp4", "rms_norm_kernel")
...
def count_llama_kernel_calls(config: LlamaModelConfig) -> Dict[str, int]:
    ...
    1. Pre-attention: fused_add_rms_norm (residual + RMSNorm) -> 1 call
    ...
    4. Pre-FFN: fused_add_rms_norm (residual + RMSNorm) -> 1 call
    ...
    Note: vLLM fuses residual add + RMSNorm into fused_add_rms_norm_kernel.
    The standalone rms_norm_kernel is only used for the final norm after all layers.
...
def calculate_llama_kernel_bytes(config: LlamaModelConfig) -> Dict[str, Dict]:
    ...
    2. fused_add_rms_norm_kernel (vLLM fused residual + norm):
    ...
    3. rms_norm_kernel (standalone, final norm only):
       - Reads: input (BF16) = tokens * hidden_dim * 2 bytes
       - Writes: output (BF16) = tokens * hidden_dim * 2 bytes
...
def calculate_vllm_kernel_bandwidths(
    ...
    """Calculate bandwidth for vLLM element-wise kernels.
    ...
    Args:
        ...
        fused_add_rms_norm_time_s: Measured time for fused_add_rms_norm kernels (vLLM)
    ...
    if fused_add_rms_norm_time_s is not None and fused_add_rms_norm_time_s > 0:
        kernel_info.append(("fused_add_rms_norm", "fused_add_rms_norm_kernel", fused_add_rms_norm_time_s))

    # Add standalone rms_norm if time provided and > 0
    if layernorm_time_s > 0:
        kernel_info.append(("rms_norm", "rms_norm_kernel (final)", layernorm_time_s))

    kernel_info.append(("silu", "act_and_mul_kernel (SiLU)", silu_time_s))
...
def generate_vllm_bandwidth_report(
    ...
    """Generate formatted bandwidth report for vLLM kernel profiling.
    ...
    lines = []
    lines.append("=" * 90)
    lines.append(f"vLLM Element-wise Kernel Bandwidth Report (B200 Peak: {peak_gbs:.0f} GB/s)")
    lines.append("=" * 90)
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description="Memory Bandwidth Profiler for vLLM Element-wise Kernels (Llama 3.1 8B)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.002191,
        help="Time for quantize (cvt_fp16_to_fp4) kernels in seconds (default: 0.002191)",
    )
    parser.add_argument(
        "--layernorm-time",
        type=float,
        default=0.00001568,
        help="Time for RMSNorm kernels in seconds (default: 0.00001568)",
    )
    parser.add_argument(
        "--silu-time",
        type=float,
        default=0.005103,
        help="Time for SiLU/act_and_mul kernels in seconds (default: 0.005103)",
    )
    parser.add_argument(
        "--fused-add-rms-norm-time",
        type=float,
        default=None,
        help="Time for fused_add_rms_norm kernels (vLLM fuses residual + norm)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Call count for fused_add_rms_norm (typically num_layers * 2)",
    )
    ...
    results, calls = calculate_vllm_kernel_bandwidths(
        config=config,
    ...
    # Generate report
    report = generate_vllm_bandwidth_report(
        results=results,
        calls=calls,
        config=config,
        peak_gbs=args.peak_bandwidth,
