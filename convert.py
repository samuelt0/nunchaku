...
def convert_to_nunchaku_transformer_block_state_dict(
    ...
    for converted_local_name, candidate_local_names in tqdm.tqdm(
        ...
        if len(weight) > 1:
            # Check if we have mixed quantization (some weights quantized, some not)
            if not all(s is None for s in scale) and not all(s is not None for s in scale):
                # Mixed quantization - handle each weight separately
                print(f"  - Detected mixed quantization for {candidate_local_names}")
                for idx, (w, local_name, s) in enumerate(zip(weight, candidate_local_names, scale)):
                    individual_name = f"{converted_local_name}_{idx}" if len(weight) > 1 else converted_local_name
                    if s is not None:
                        # This weight is quantized
                        print(f"    - Converting quantized weight {local_name}")
                        individual_bias = bias[idx] if bias[idx] is not None else None
                        individual_subscale = subscale[idx] if subscale[idx] is not None else None
                        individual_smooth = smooth_dict.get(f"{block_name}.{local_name}", None)
                        individual_branch = branch_dict.get(f"{block_name}.{local_name}", None)
                        if individual_branch is not None:
                            individual_branch = (individual_branch["a.weight"], individual_branch["b.weight"])
                        
                        # Determine if smooth should be fused
                        smooth_fused = "out_proj" in converted_local_name and smooth_dict.get("proj.fuse_when_possible", True)
                        shift = candidates.get(f"{block_name}.{local_name[:-7]}.shift", None) if local_name.endswith(".weight") else None
                        ...
                        update_state_dict(
                            ...
                            convert_to_nunchaku_w4x4y16_linear_state_dict(
                                weight=w,
                                scale=s,
                                bias=individual_bias,
                                smooth=individual_smooth,
                                lora=individual_branch,
                                shift=shift,
                                smooth_fused=smooth_fused,
                                float_point=float_point,
                                subscale=individual_subscale,
                            ),
                            prefix=individual_name,
                        )
                    else:
                        # This weight is unquantized
                        print(f"    - Copying unquantized weight {local_name}")
                        converted[f"{individual_name}.weight"] = w.clone().cpu()
                        if bias[idx] is not None:
                            converted[f"{individual_name}.bias"] = bias[idx].clone().cpu()
                # Skip the rest of the processing for this group
                continue
            
            # No mixed quantization - process as before
            bias = None if all(b is None for b in bias) else torch.concat([b for b in bias if b is not None], dim=0)
            if all(s is None for s in scale):
                scale = None
            elif scale[0].numel() == 1:  # switch from per-tensor to per-channel scale
            ...
            if all(s is None for s in subscale):
                ...
                # Check for partial subscale quantization
                non_none_subscales = [s for s in subscale if s is not None]
                if len(non_none_subscales) != len(subscale):
                    print(f"    WARNING: Missing subscales for some weights")
                    subscale = None
                else:
                    subscale = torch.concat(subscale, dim=0)
...
def convert_to_nunchaku_wan_transformer_block_state_dict(
    ...
    local_name_map = {
        ...
        "qkv_proj": ["attn1.to_q", "attn1.to_k", "attn1.to_v"],
        "norm_q": "attn1.norm_q",
        "norm_k": "attn1.norm_k",
...
def convert_to_nunchaku_wan_state_dicts(
    state_dict: dict[str, torch.Tensor],
    scale_dict: dict[str, torch.Tensor],
    smooth_dict: dict[str, torch.Tensor],
    branch_dict: dict[str, torch.Tensor],
    ...
    """Convert Wan model state dicts to Nunchaku format."""
    block_names: set[str] = set()
    other: dict[str, torch.Tensor] = {}
    
    # Identify transformer blocks (Wan uses "blocks" instead of "transformer_blocks")
    ...
    print(f"Converting {len(block_names)} Wan transformer blocks...")
    
    converted: dict[str, torch.Tensor] = {}
    for block_name in block_names:
        update_state_dict(
            converted,
            convert_to_nunchaku_wan_transformer_block_state_dict(
                state_dict=state_dict,
                scale_dict=scale_dict,
                smooth_dict=smooth_dict,
                branch_dict=branch_dict,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default=None, help="path to the model (used to infer model name).")
    ...
    if args.model_name is None:
        if args.model_path is not None:
            ...
            # Try to infer model name from checkpoint structure
            test_dict_path = os.path.join(args.quant_path, "model.pt")
            if os.path.exists(test_dict_path):
                test_dict = torch.load(test_dict_path, map_location="cpu", weights_only=False)
                # Check if it's a Wan model or Flux model based on block naming
                has_blocks = any(key.startswith("blocks.") for key in test_dict.keys())
                has_transformer_blocks = any(key.startswith("transformer_blocks.") for key in test_dict.keys())
                if has_blocks and not has_transformer_blocks:
                    model_name = "wan-model"
                    print("Detected Wan model structure based on 'blocks.' prefix")
                elif has_transformer_blocks:
                    model_name = "flux-model"
                    print("Detected Flux model structure based on 'transformer_blocks.' prefix")
                else:
                    raise ValueError("Could not determine model type from checkpoint structure. Please provide --model-name")
                del test_dict
            else:
    ...
    if not is_flux and not is_wan:
        # Try to detect from checkpoint structure if model name doesn't clearly indicate type
        test_dict_path = os.path.join(args.quant_path, "model.pt")
        if os.path.exists(test_dict_path):
            test_dict = torch.load(test_dict_path, map_location="cpu", weights_only=False)
            has_blocks = any(key.startswith("blocks.") for key in test_dict.keys())
            ...
            if has_blocks and not has_transformer_blocks:
                is_wan = True
                print("Auto-detected Wan model based on checkpoint structure")
            elif has_transformer_blocks:
                is_flux = True
                print("Auto-detected Flux model based on checkpoint structure")
            del test_dict
    
    if not is_flux and not is_wan:
        raise ValueError(f"Model type could not be determined for '{model_name}'. Please ensure model name contains 'flux' or 'wan'.")
    ...
    if is_wan:
        print(f"Converting Wan model: {model_name}")
        converted_state_dict, other_state_dict = convert_to_nunchaku_wan_state_dicts(
            state_dict=state_dict,
            scale_dict=scale_dict,
            smooth_dict=smooth_dict,
            branch_dict=branch_dict,
        ...
        print(f"Converting Flux model: {model_name}")
